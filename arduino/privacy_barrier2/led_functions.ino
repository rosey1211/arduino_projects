...
{
   float scaled_brightness = (float)g_currentBrightnessLevel/(float)g_maxBrightnessLevel;
   int new_red;
   int new_green;
   int new_blue;
...
void setupColorRange()
{
   ...
   //g_colorSample = (CRGB *)malloc(sizeof(CRGB)*6*256/g_color_skip);
   int color_index=0;
   // start increasing green
   // moving to yellow from red
   for(int g=0;g<=255;g+=g_color_skip)
   {
      g_colorSample[color_index].red = 255;
      g_colorSample[color_index].green=g;
      g_colorSample[color_index].blue=0;
      color_index++;
   }

   // start decreasing red
   // moving from yellow to green
   for(int r=g_color_skip;r<=255;r+=g_color_skip)
   {
      g_colorSample[color_index].red = 255-r;
      g_colorSample[color_index].green=255;
      g_colorSample[color_index].blue=0;
      color_index++;
   }
   ...
   for(int b=g_color_skip;b<=255;b+=g_color_skip)
   {
      g_colorSample[color_index].red = 0;
      g_colorSample[color_index].green=255;
      g_colorSample[color_index].blue=b;
      color_index++;
   }

   // start decreasing green
   // moving from turquoise to blue
   for(int g=g_color_skip;g<=255;g+=g_color_skip)
   {
      g_colorSample[color_index].red = 0;
      g_colorSample[color_index].green=255-g;
      g_colorSample[color_index].blue=255;
      color_index++;
   }
   ...
   for(int r=g_color_skip;r<=255;r+=g_color_skip)
   {
      g_colorSample[color_index].red = r;
      g_colorSample[color_index].green=0;
      g_colorSample[color_index].blue=255;
      color_index++;
   }
   ...
   for(int b=g_color_skip;b<=255;b+=g_color_skip)
   {
      g_colorSample[color_index].red = 255;
      g_colorSample[color_index].green=0;
      g_colorSample[color_index].blue=255-b;
      color_index++;
   }

   g_total_number_of_colors = color_index;
   if(debugLevel>2)
   {
      for(int i=0;i<color_index;i++)
      {
        ...
        Serial.print("] : ");
        Serial.print("red = ");
        Serial.print(g_colorSample[i].red);
        Serial.print(" : green = ");
        Serial.print(g_colorSample[i].green);
        Serial.print(" : blue = ");
        Serial.print(g_colorSample[i].blue);
        Serial.print("\n");
      }
   }
}
...
void singleColor()
{
  if(debugLevel>2)
  {
    Serial.print("max_color_level = ");
    Serial.print(g_maxColorLevel);
    Serial.print(" : current_color_level = ");
   ...
   CRGB currentColor = g_colorSample[local_color_index];
   CRGB adjustedColor;
   adjustColorBasedOnBrightness(currentColor,adjustedColor);
   for(int i=0;i<NUM_LEDS;i++)
   {
...
void temporalRainbow()
{
  ...
  for(int i=0;i<NUM_LEDS;i++)
    {
      g_leds[i].red = adjusted_color.red;
      g_leds[i].green = adjusted_color.green;
      g_leds[i].blue = adjusted_color.blue;
    }
}
...
bool readLEDProgramButton()
{
  bool program_changed=false;
  bool currentButton = digitalRead(LED_PROGRAM_TOGGLE_PIN);
  if(currentButton==HIGH && g_lastLEDProgramButton==LOW)
  {
    g_currentLEDProgramID = (g_currentLEDProgramID+1)%MAX_NUMBER_LED_PATTERNS;
    Serial.print("led program = ");
    Serial.print(g_currentLEDProgramID);
    Serial.print("\n");
    g_lastLEDProgramButton = HIGH;
    resetLEDs();
    program_changed=true;
  }
  if(currentButton==LOW && g_lastLEDProgramButton==HIGH)
  {
    // reset
    g_lastLEDProgramButton=LOW;
  }
...
void runLEDProgram()
{ 
  if(g_currentLEDProgramID==SINGLE_COLOR)
  {
    singleColor();
  }
  else if(g_currentLEDProgramID==TEMPORAL_RAINBOW)
  {
    temporalRainbow();
  }
  else if(g_currentLEDProgramID==SAME_COLOR_RANDOM_LED_BLOCKS)
  {
    sameColorRandomBlocks();
  }
  else if(g_currentLEDProgramID==AVALANCHE_COLOR_RANDOM_LED_BLOCKS)
  {
    CRGB color1;
    CRGB color2;
    ...
    color2.green = 35;
    color2.blue = 200;
    twoColorRandomBlocks(color1,color2);
  }
  else if(g_currentLEDProgramID==BRONCOS_COLOR_RANDOM_LED_BLOCKS)
  {
    CRGB color1;
    CRGB color2;
    color1.red = 60;
  ...
  else if(g_currentLEDProgramID==NUGGETS_COLOR_RANDOM_LED_BLOCKS)
  {
    CRGB color1;
    CRGB color2;
  ...
  else if(g_currentLEDProgramID==CUBS_COLOR_RANDOM_LED_BLOCKS)
  {
    CRGB color1;
    CRGB color2;
    color1.red = 0;
  ...
  else if(g_currentLEDProgramID==TWO_COLOR_RANDOM_LED_BLOCKS1)
  {
    CRGB color1;
    CRGB color2;

    // note that the red is really green
    color1.red = 255;
    color1.green = 0;
    color1.blue = 100;
  ...
  else if(g_currentLEDProgramID==TWO_COLOR_RANDOM_LED_BLOCKS2)
  {
    CRGB color1;
    CRGB color2;
  ...
  else if(g_currentLEDProgramID==DIFFERENT_COLOR_RANDOM_LED_BLOCKS)
  {
    differentColorRandomBlocks();
  }
 else if(g_currentLEDProgramID==STATIC_WHITE)
  {
    staticWhite();
  }
#if 0
  for(int i=0;i<NUM_LEDS;i++)
  {
    if(g_leds[i].blue>1)
    {
      ...
      Serial.print(i);
      Serial.print("]: red = ");
      Serial.print(g_leds[i].red);
      Serial.print(" : green = ");
      Serial.print(g_leds[i].green);
      Serial.print(" : blue = ");
      Serial.print(g_leds[i].blue);
      Serial.print("\n");
    }
  }
  Serial.print("\n\n\n");
