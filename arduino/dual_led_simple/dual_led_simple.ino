...
#define UPPER_NUM_LEDS    40
CRGB upper_leds[UPPER_NUM_LEDS];

#define LOWER_NUM_LEDS    60
...
{
  fill_solid( upper_leds, UPPER_NUM_LEDS,temporal_color);
  if(temporal_color.red==255 && temporal_color.green<255 && temporal_color.blue==0)
  {
    // moving towards yellow from red
    if(debug)
...
void rainbow() 
{
  ...
  fill_rainbow( upper_leds, UPPER_NUM_LEDS, gHue, 7);
}
...
void addGlitter( fract8 chanceOfGlitter) 
{
  if( random8() < chanceOfGlitter) {
    upper_leds[ random16(UPPER_NUM_LEDS) ] += CRGB::White;
  }
}
...
void confetti() 
{
  ...
  fadeToBlackBy( upper_leds, UPPER_NUM_LEDS, 10);
  int pos = random16(UPPER_NUM_LEDS);
  upper_leds[pos] += CHSV( gHue + random8(64), 200, 255);
}

void sinelon()
{
  // a colored dot sweeping back and forth, with fading trails
  fadeToBlackBy( upper_leds, UPPER_NUM_LEDS, 20);
  int pos = beatsin16( 13, 0, UPPER_NUM_LEDS-1 );
  upper_leds[pos] += CHSV( gHue, 255, 192);
}
...
void bpm()
{
  ...
  for( int i = 0; i < UPPER_NUM_LEDS; i++) { //9948
    upper_leds[i] = ColorFromPalette(palette, gHue+(i*2), beat-gHue+(i*10));
  }
}
...
void juggle() {
  ...
  uint8_t dothue = 0;
  for( int i = 0; i < 8; i++) {
    upper_leds[beatsin16( i+7, 0, UPPER_NUM_LEDS-1 )] |= CHSV(dothue, 200, 255);
    dothue += 32;
  }
}
